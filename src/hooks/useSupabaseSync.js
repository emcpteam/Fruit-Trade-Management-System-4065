      ...
      try {
        ...
        try {
          ...
          if (testError && testError.code === '42P01') {
            // Table doesn't exist, that's ok for local development
            console.log('Tables not found, using local data')
            setIsInitialized(true)
            return
          }
          ...
          if (testError && (testError.code === 'PGRST301' || testError.message.includes('JWT'))) {
            ...
            console.warn('Supabase authentication error, using local data:', testError.message)
            setIsInitialized(true)
            return
          }
